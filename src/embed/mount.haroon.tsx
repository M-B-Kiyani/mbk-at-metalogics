...
import { HaroonEmbedApp } from "./HaroonEmbedApp";
...
// Deliberately distinct from MBK's mount id/attribute (mbk-profile-root /
// data-mbk-profile-mount) so both embeds can be placed on the same
// WordPress page — e.g. a "Team" page with two mount divs — without
// colliding on id or accidentally double-mounting into each other's div.
const MOUNT_ATTR = "data-haroon-profile-mount";
const DEFAULT_MOUNT_ID = "haroon-profile-root";
...
function init() {
  // Primary target: an explicit mount point, e.g. <div id="haroon-profile-root">
  // Also supports data-haroon-profile-mount="" on any element, and multiple
  ...
  if (targets.size === 0) {
    console.warn(
      `[haroon-profile] No mount point found. Add <div id="${DEFAULT_MOUNT_ID}"></div> ` +
